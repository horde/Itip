...
implements Horde_Itip_Event
{
     ...
     * Constructor.
     *
     * @param Horde_Icalendar_Vtodo $vevent The iCalendar object that will be
     *                                       wrapped by this instance.
     */
    public function __construct(Horde_Icalendar_Vtodo $vevent)
    {
        $this->_vevent = $vevent;
    }
     ...
     *
     * @todo H6, either change name of method, or add method specific to Vtodo.
     * @return Horde_Icalendar_Vtodo The wrapped todo.
     */
    ...
    public function copyEventInto(Horde_Itip_Event $event)
    {
        ...
        $this->copyPercentComplete($event);
    }
    ...
    private function copyPercentComplete(Horde_Itip_Event $itip)
    {
        $itip->setPercentComplete($this->getPercentComplete());
    }

    /**
     * Set the percentage complete for the vTodo.
     *
     * @param integer $percent  The percentage.
     */
    private function setPercentComplete($percent)
    {
        $this->_vevent->setAttribute('PERCENT-COMPLETE', $percent);
    }
